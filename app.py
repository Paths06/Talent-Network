...
def extract_entities_with_gemini(text_chunk: str, model_name: str):
    ...
    try:
        ...
        # This will catch errors if configure() was not called, which is handled in the main UI
        st.error(f"Error initializing Gemini model. Is the API key configured correctly? Details: {e}")
        return None
...
# --- FIX IS HERE: Use a boolean flag to track API key status ---
...
if uploaded_file is not None:
    ...
    if file_identifier != st.session_state.current_file_id:
    ...
    with col_btn:
        if st.button(f"Process Next Batch ({min(batch_size, items_left)} items)", disabled=(items_left == 0), type="primary"):
            # --- FIX IS HERE: Check the boolean flag instead of the non-existent attribute ---
