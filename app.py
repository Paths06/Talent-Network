...
if GENAI_AVAILABLE:
    ...
    model = _create_model(st.session_state.gemini_model_name) # Pass the selected model name
        ...
        if uploaded_file is not None:
            ...
            if document_content and GENAI_AVAILABLE and gemini_api_key:
                if st.button("Extract Entities"):
                    # --- Throttling Logic Before API Call ---
                    current_rpm_limit = GEMINI_MODELS[st.session_state.gemini_model_name]["rpm_limit"]
                    # Calculate ideal delay per request based on RPM, add a small buffer
                    ...
                    if time_since_last_extraction < delay_per_request:
                        ...
                        st.info(f"Rate limit active for {GEMINI_MODELS[st.session_state.gemini_model_name]['display_name']}. Waiting {time_to_wait:.1f} seconds before extraction...")
                        time.sleep(time_to_wait)
                        
                    # Update the last extraction time immediately before the call
